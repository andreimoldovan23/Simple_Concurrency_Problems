...
public class Polynomial {
    ...
    public Polynomial(String name, Integer value) {
        ...
        coefficients = Utils.getRandomCoefficients(PropertiesLoader.getMaxCoefficient(), rank);
        log.info("Polynomial created: {}, rank: {}", name, rank);
    }

    public List<Integer> serializeAsArray() {
        List<Integer> array = new ArrayList<>();
        array.add(value);
        array.add(rank);
        array.addAll(getCoefficients());
        return array;
    }

    public List<Long> getListOfTerms() {
        AtomicReference<Integer> ref = new AtomicReference<>(rank);
        return coefficients.stream()
                .map(cof -> cof * Utils.pow(value, ref.getAndUpdate(x -> x - 1)))
